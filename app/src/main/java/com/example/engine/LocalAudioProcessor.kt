 ...
 * designed to spot the acoustic pattern of "Hey Burmese Universal AI" or "Burmese Universal AI".
...
class LocalAudioProcessor {
    ...
    // Syllable/Pattern Detection State Machine for "Hey Burmese Universal AI"
    private enum class KwsState {
        IDLE,
        SYLLABLE_1,      // Detecting "Hey" (First vocal energy burst)
        GAP,             // Detecting the brief silence/dip between "Hey" and "Burmese Universal AI"
        SYLLABLE_2       // Detecting "Burmese Universal AI" (Second elongated vocal energy burst)
    }
    ...
    fun processAudioFrame(shortBuffer: ShortArray, length: Int): Boolean {
        ...
        when (currentState) {
            ...
            KwsState.GAP -> {
                if (!isSpeechLike) {
                    ...
                    if (durationMs in 50..400) {
                        ...
                        stateFrameCount = 1
                        Log.d(tag, "🎙️ KWS State Change: GAP -> SYLLABLE_2 ('Burmese Universal AI') (Gap duration: $durationMs ms)")
                    } else {
                        currentState = KwsState.IDLE
            ...
            KwsState.SYLLABLE_2 -> {
                if (isSpeechLike) {
                    // Maximum duration of "Burmese Universal AI" syllable is 1000ms
                    if (durationMs > 1000) {
                        currentState = KwsState.IDLE
                    ...
                    if (durationMs in 250..1000) {
                        ...
                        if (currentTime - lastTriggerTime > triggerCooldownMs) {
                            ...
                            triggered = true
                            Log.d(tag, "🎉 LOCAL KEYWORD SPOTTED SUCCESS: 'Hey Burmese Universal AI' detected locally!")
                        }
                        currentState = KwsState.IDLE
                    } else {
                        currentState = KwsState.IDLE
