...
class WakeWordEngine {
    ...
    fun startListening(onWakeWordDetected: () -> Unit) {
        ...
        isListening = true
        Log.d(tag, "Wake word engine active. Continuously capturing audio for 'Hey Burmese Universal AI' / 'Burmese Universal AI'...")
                    ...
                    while (isActive && isListening) {
                        ...
                        if (readResult > 0) {
                            ...
                            if (interpreter != null) {
                                ...
                                if (localTriggered) {
                                    Log.d(tag, "Triggering Burmese Universal AI via Local Audio Processing library!")
    ...
    private suspend fun runSimulatedAudioListening() {
        ...
        while (currentCoroutineContext().isActive && isListening) {
            ...
            if (elapsedSeconds == 8 || (elapsedSeconds > 8 && (elapsedSeconds - 8) % 44 == 0)) {
                Log.d(tag, "Auto-triggering keyword 'Hey Burmese Universal AI' via simulated engine to initiate hands-free recognition.")
                _isWakeWordDetectedFlow.value = true
                onWakeWordDetectedListener?.invoke()
                _isWakeWordDetectedFlow.value = false
            }
    ...
    private fun runInference() {
        ...
        try {
            ...
            // Output shape: [1, 2] -> index 0: silence/others, index 1: "Hey Burmese Universal AI"
            ...
            if (wakeWordProb > detectionThreshold) {
                Log.d(tag, "TFLite Wake Word Detected: 'Hey Burmese Universal AI' with probability $wakeWordProb")
                _isWakeWordDetectedFlow.value = true
                onWakeWordDetectedListener?.invoke()
                _isWakeWordDetectedFlow.value = false
            }
    ...
    fun simulateWakeWordMatch() {
        ...
        Log.d(tag, "Simulating instant Burmese Wake Word match for Burmese Universal AI!")
        _isWakeWordDetectedFlow.value = true
        onWakeWordDetectedListener?.invoke()
        _isWakeWordDetectedFlow.value = false
    }
